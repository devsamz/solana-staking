...
describe("staking", async () => {
  ...
  // Get PDA of token mint and mint authority with seeds
  ...
  // Airdrop tokens to user account
  ...
  // Attempt to stake more tokens than the user has, should fail
  ...
  // Attempt to unstake more tokens than the user has staked, should fail
  it("Should not unstake too many tokens", async () => {
    ...
    // Check if user's token balance and staking vault remain unchanged
    let account = await getAccount(provider.connection, userTokenAccount);
    expect(account.amount).to.eql(BigInt(25));
  ...
  // Unstake remaining tokens from staking vault back to user's account
